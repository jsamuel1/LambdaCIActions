...
- 🎯 A repo with docker + node jobs routes each to the right flavor with no YAML edits beyond adding LCA labels.

## M4 — Web UI & Management API
Operator visibility + control.
- `MgmtStack` API + `WebStack` SPA + GitHub OAuth.
- Screens: Dashboard, Repos, Repo/Workflow detail, Runs, Run detail (logs), Flavors, Settings.
- Run history + CloudWatch log viewer; live status via polling.
