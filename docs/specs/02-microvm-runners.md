...
The `node`, `python`, `go` and `rust` flavors also carry a **C toolchain**
(`build-essential`). A prebaked runtime does not make a job self-sufficient: an `npm ci` that
falls through to **node-gyp** (npm's path whenever a dependency ships no prebuilt binary for the
platform — and arm64 is where prebuilds are most often absent), a `pip install` of a source-only
sdist (arm64 wheels are still missing for plenty of packages), a `go build` touching cgo, and
every `cargo` link step all shell out to a compiler the shared apt line does not provide.
Verified in a container: that line yields no `gcc`/`cc`/`g++`/`make`/`ld` (only `python3`, which
node-gyp also needs). Without it the failure lands mid-job (`command 'gcc' failed: No such file
or directory`, or `gyp ERR! ... not found: make`) after the download has already been paid.
`base`/`java`/`docker` deliberately skip it — `base` carries no language runtime to compile
against, Temurin builds consume published JARs, and a docker job compiles inside its own
container; it is ~200 MB of snapshot each. The prebuilt CPython ships its own headers, so no
`python3-dev` is needed on top.
