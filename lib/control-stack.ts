...
export class ControlStack extends Stack {
  constructor(scope: Construct, id: string, props: ControlStackProps) {
    ...
    // job end, read its own microvmId back off the run row for self-terminate (ADR-018).
    table.grantReadData(microvmExecRole);
    // Self-terminate (ADR-018): the hook calls terminate-microvm on ITSELF at job end so
