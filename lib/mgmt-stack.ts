...
export interface MgmtStackProps extends StackProps {
  ...
  publicOrigin?: string;
  /**
   * App-config broker (ControlStack, ADR-033). The console λ invokes it to verify the GitHub
 ...
 *     plane, reachable only via `lambda:InvokeFunction` on that one ARN (ADR-033).
 */
export class MgmtStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: MgmtStackProps) {
    ...
    // App-config broker (ADR-033): the console's ONLY route to App-PEM-requiring GitHub calls
