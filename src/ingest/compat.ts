...
export function analyzeCompat(job: ParsedJob, resolution: FlavorResolution): CompatResult {
  ...
  //
  // ADOPT MODE passes this gate silently BY CONSTRUCTION, and that is deliberate (ADR-030 ×
  // ADR-039). An adopt-mode job routes `ubuntu-latest` → `base`: (1) `ubuntu-latest` is not a
  // catalog label, so source 1 contributes nothing, and (2) a docker signal upgrade sets
  // `replaced: 'base'`, whose capability list is EMPTY, so source 2 contributes nothing either.
  // A job that never named a toolchain cannot have one dropped. This is why
  // `ADOPT_LABEL_FLAVORS` must keep mapping every standard label to a capability-less flavor:
  // mapping `ubuntu-latest → node` (rejected in `adopt.ts` for a different reason — the label
