 ...
 * (ADR-033): SSM's parameter history holds the previous VALUES, so we never copy one.
 */
 ...
 * broker holds the IAM grant for this on secret paths (ADR-033) — it is how rollback
 ...
 * management λ has NO `ssm:PutParameter` grant of any kind (ADR-025 + ADR-033).
 *
 ...
 * (ADR-033). A missing parameter is treated as already-undone.
 */
