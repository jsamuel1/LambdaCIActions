...
- **Deploy-target pin (ADR-018, extended by ADR-047):** deploy-touching commands (cdk deploy/diff, `build:images`, `app:create`, `backfill:installs`) REQUIRE a pin of `LCA_DEPLOY_ACCOUNT` + `LCA_DEPLOY_REGION` — from a gitignored `.env.local` (copy `.env.local.example`) on a workstation, or from **the process environment** in CI, which has no gitignored file to read (`.env.local` wins when both exist). Either way they verify the ambient credentials actually resolve to that account and refuse on mismatch — there is no "skip in CI" branch; the identity match, not the pin's provenance, is the safety property. Optional `LCA_DEPLOY_ENV=dev|prod` binds the env selector too (`-c env=`/`--env` must match), so prod-namespaced resources/image ARNs/App secrets can't land in the dev account. Credential-less `cdk synth` and `--dry-run` are exempt — except `backfill:installs`, whose dry run reads the live table and so is pinned too (ADR-037).
- **CD is dogfooded (ADR-047):** `.github/workflows/deploy.yml` deploys `LCA-Mgmt-dev` + `LCA-Web-dev` from our own microVM runners under a GitHub-OIDC role (`LCA-Deploy-<env>`, deployed by hand and never in CD's allowlist). CI must never deploy `LCA-Image-*`/`LCA-Control-*`/`LCA-Data-*`/`LCA-Deploy-*`, never `--all`, and every CD `cdk deploy` needs `--exclusively` (Mgmt depends on Data+Control, which CDK would otherwise redeploy — killing the runner running the job). Workstation `cdk deploy` is the documented escape hatch (`docs/DEPLOY-M4.md`).
